...
- [ ] **Frontend: Create Onboarding Screen Components** (AC: 1, 2, 3, 4, 7, 10, 11, 12)
  - [ ] Create `OnboardingScreen.tsx` with multi-step navigation
  - [ ] Implement `WelcomeStep.tsx` component with illustration and CTA
  - [ ] Implement `FreeStarterSliceStep.tsx` component with gift messaging
  - [ ] Implement `HowItWorksStep.tsx` component with feature cards
  - [ ] Add progress indicator component for step navigation
  - [ ] Implement skip functionality with confirmation modal
  - [ ] Add proper accessibility attributes and screen reader support
  - [ ] Ensure design system compliance using design.json specifications

- [ ] **Frontend: Onboarding Navigation & State Management** (AC: 7, 8, 9)
  - [ ] Create onboarding store using Zustand for step management
  - [ ] Implement step navigation logic (next, previous, skip)
  - [ ] Add completion handler that redirects to dashboard
  - [ ] Integrate with Expo Router for proper navigation flow
  - [ ] Handle onboarding completion state persistence

- [ ] **Backend: Automatic Wallet Creation API** (AC: 5)
  - [ ] Create `POST /auth/create-wallet` endpoint
  - [ ] Integrate Thirdweb SDK for in-app wallet creation
  - [ ] Store wallet address in user profile
  - [ ] Add proper error handling for wallet creation failures
  - [ ] Implement wallet creation as part of registration flow

- [ ] **Backend: Free Starter Investment Logic** (AC: 6, 9)
  - [ ] Create `POST /onboarding/starter-investment` endpoint
  - [ ] Define default starter basket (low-risk, beginner-friendly)
  - [ ] Implement automatic investment allocation logic
  - [ ] Create portfolio holding record for starter investment
  - [ ] Add transaction record for the free starter slice
  - [ ] Ensure starter investment appears in user's portfolio
